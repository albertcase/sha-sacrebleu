 ...
 */
if ( ! defined( 'WPB_VC_VERSION' ) ) define( 'WPB_VC_VERSION', '4.3.1' );
/**
...
class Vc_Manager {
	...
	/**
	 * Gets actions classes instance.
	 *
	 * @since 4.4
	 * @access public
	 *
	 * @return Vc_Interactions
	 */
	public function interactions() {
		if ( ! isset( $this->factory['interactions'] ) ) {
			do_action( 'vc_before_init_interactions' );
			require_once $this->path( 'CORE_DIR', 'class-vc-interactions.php' );
			$interactions = new Vc_Interactions();
			$this->factory['interactions'] = $interactions;
			do_action( 'vc_after_init_interactions' );
		}
		return $this->factory['interactions'];
	}
	/**
...
/**
 * Activation hook called after VC activation as WordPress plugin.
 *
 */
function vc_js_composer_activate() {
	vc_settings()->initAdmin();
	vc_settings()->buildCustomColorCss();
	vc_settings()->buildCustomCss();
}

add_action( 'after_switch_theme', 'vc_js_composer_on_theme_switch' );
function vc_js_composer_on_theme_switch() {
	add_action( 'after_setup_theme', 'vc_js_composer_activate', 20 );
}

// register_activation_hook( __FILE__, 'vc_js_composer_activate' );
