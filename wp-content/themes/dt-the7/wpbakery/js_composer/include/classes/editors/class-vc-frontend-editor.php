...
Class Vc_Frontend_Editor implements Vc_Editor_Interface {
	...
	public function buildEditablePage() {
		...
		add_filter( 'the_title', array( &$this, 'setEmptyTitlePlaceholder' ) );
	...
	function renderEditor() {
		...
		if ( $this->post && $this->post->post_status === 'auto-draft' ) {
			$post_data = array( 'ID' => $this->post_id, 'post_status' => 'draft', 'post_title' => ' ' ); // , 'post_title' => __('No title', LANGUAGE_ZONE)
	...
	*/
	...
	function setEmptyTitlePlaceholder( $title ) {
		return !is_string($title) || strlen($title)==0 ? __('(no title)', LANGUAGE_ZONE) : $title;
	}
